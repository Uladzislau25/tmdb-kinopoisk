...
cards.forEach((card, index) => { card.style.display = index < maxCards ? 'block' : 'none'; });
// 3️⃣ Рассчитываем равный gap между видимыми карточками

let gap = minGap; if (maxCards > 1) {
...
cards.forEach((card, index) => {
    if (index < maxCards) {
        card.style.marginRight = index === lastVisibleIndex ? '0px' :
            `${gap}px`;
    } else { card.style.marginRight = '0px';
    }
});
};